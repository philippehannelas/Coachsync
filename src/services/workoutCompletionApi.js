...
export const getWorkoutHistory = async (days = 30) => {
  const response = await fetch(
    `${API_BASE_URL}/api/customer/workouts/history?days=${days}`,
    {
      headers: getAuthHeaders()
    }
  );
...
export const getCustomerStats = async () => {
  const response = await fetch(
    `${API_BASE_URL}/api/customer/stats`,
    {
      headers: getAuthHeaders()
    }
  );
...
export const getCustomerWorkoutHistory = async (customerId, days = 30) => {
  const response = await fetch(
    `${API_BASE_URL}/api/coach/customers/${customerId}/workout-history?days=${days}`,
    {
      headers: getAuthHeaders()
    }
  );
...
export const getCustomerStatsCoach = async (customerId) => {
  const response = await fetch(
    `${API_BASE_URL}/api/coach/customers/${customerId}/stats`,
    {
      headers: getAuthHeaders()
    }
  );
