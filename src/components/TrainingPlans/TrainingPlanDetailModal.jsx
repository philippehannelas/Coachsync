 ...
 * 
 * FIXED VERSION - Fetches exercises from API instead of reading from plan.exercises
 */
function TrainingPlanDetailModal({ plan, onClose, onEdit }) {
  ...
  useEffect(() => {
    const loadExercises = async () => {
      console.log('🔍 TrainingPlanDetailModal - Loading exercises for plan:', plan.id);
      
      if (!plan?.id) {
        console.error('❌ TrainingPlanDetailModal - No plan ID found!');
        setLoading(false);
        return;
      }
      ...
      try {
        setLoading(true);
        const url = `https://coachsync-pro.onrender.com/api/coach/training-plans/${plan.id}/exercises`;
        console.log('📡 TrainingPlanDetailModal - Fetching from:', url);
        ...
        console.log('📊 TrainingPlanDetailModal - Response status:', response.status);
        
        if (response.ok) {
          const data = await response.json();
          console.log('✅ TrainingPlanDetailModal - Loaded exercises:', data.length, 'exercises');
          setExercises(data || []);
        } else {
